...
// GetSecretWithRetry provides a mock function with given fields: retries, waitTime, path, keys
func (_m *SecretProvider) GetSecretWithRetry(retries int, waitTime time.Duration, path string, keys ...string) (map[string]string, error) {
	_va := make([]interface{}, len(keys))
	for _i := range keys {
		_va[_i] = keys[_i]
	}
	var _ca []interface{}
	_ca = append(_ca, retries, waitTime, path)
	_ca = append(_ca, _va...)
	ret := _m.Called(_ca...)

	var r0 map[string]string
	if rf, ok := ret.Get(0).(func(int, time.Duration, string, ...string) map[string]string); ok {
		r0 = rf(retries, waitTime, path, keys...)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(map[string]string)
		}
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(int, time.Duration, string, ...string) error); ok {
		r1 = rf(retries, waitTime, path, keys...)
	} else {
		r1 = ret.Error(1)
	}
